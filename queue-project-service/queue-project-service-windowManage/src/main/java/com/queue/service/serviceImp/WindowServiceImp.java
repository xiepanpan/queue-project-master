...
public class WindowServiceImp implements WindowService {
    ...
    public ResultBean getWindowList(Window window) throws Exception {
        ...
        List<Window> windows=windowMapper.selectWindow(window);
        for (Window window1:windows){
            if (window1.getBusinessNowNum().equals(window1.getBusinessNum())){
                window1.setWindowStatus(1);
                windowMapper.updateWindow(window1);
            }
        }
        windows=windowMapper.selectWindow(window);
        resultBean.setDataList(windows);
